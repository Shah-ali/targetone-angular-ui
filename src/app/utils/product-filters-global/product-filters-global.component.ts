...
@Component({
  selector: 'app-product-filters-global',
  templateUrl: './product-filters-global.component.html',
  styleUrls: ['./product-filters-global.component.scss']
})

export class ProductFiltersGlobalComponent implements OnInit {
  ...
  parameterType: any;
  keyValueArray: any = [{id:this.uuidForSelect,key:'', value:'',options: this.parameterStringType, add: false,fieldVal:"-1",operatorVal:"-1",multiSelectVal:[]}];
 ...
 @Input() inputParamSelected:any = [];
  ...
  @Input() sendDataToProductFilters:any;
    ...
    if(this.noofParameterFiltersArry !== undefined){
      if(this.noofParameterFiltersArry.length > 0){
        this.noofParameterFiltersArry.forEach((item,i) => {
          ...
          this.parameterValueArray.emit(this.noofParameterFiltersArry);
          setTimeout(() => {                
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['sendDataToProductFilters']) {
      this.dataModelSelectedObj = this.sendDataToProductFilters;
      let res = this.dataModelSelectedObj;
      if(res.id !== undefined){       
        if(res.isEdit){
          this.editModeEnabled = res.isEdit;
          this.noofParameterFiltersArry = res.savedObj.parameterValues;
          if(!changes['sendDataToProductFilters'].firstChange && changes['sendDataToProductFilters'].previousValue !== undefined){
            this.populateFieldsOptionMethod(); 
          }//else{
          //   this.populateFieldsOptionMethod(); 
          // }
