...
@Component({
  selector: 'app-journey-non-customer-dme',
  templateUrl: './journey-non-customer-dme.component.html',
  styleUrls: ['./journey-non-customer-dme.component.scss']
})
export class JourneyNonCustomerDMEComponent implements OnInit {
  ...
  constructor(private bsModalRef: BsModalRef, private httpService: HttpService, private clipboard: Clipboard, private translate: TranslateService, private shareService: SharedataService, private ngZone:NgZone, private dataService: DataService) {
    this.agGridLocaleLabels = {
  ...
  loadDmeNonCustomerMethod(){
    this.shareService.isJourneyDMENonCustomerEditMode.subscribe((res:any) => {
  ...
  clearSelectionRowMethod(){
    ...
    this.viewParameterSectionEnabled = false;
    this.shareService.selectedRowCheckedbox.next({name:"",blockName:'dmeNonCustomer',editMode:false});
    this.parameterValueArray = undefined;
    setTimeout(() => {
  ...
  redirectToLayoutPage(evt){
    let validCheck = this.multiRowNextValidayionMethod();
    if(validCheck){
      // Swal.fire({
      //   titleText: 'No Filter are defined, please define the any one parameters to continue',
...
multiRowNextValidayionMethod(){
  ...
  let objParams = {};
   ...
   // let parameterValues = JSON.stringify(objParams);
    if(this.parameterValueArray !== undefined){
  ...
  insertApiRow(): void {
   ...
   if(this.multiArrayPath){
    dynamicTemplateInsert = "<dynamic-content id='"+uuid+"' type='dme' layout='"+layout+"' modelName='" + this.selectedDmeDataObj.modelNameHash + "' modelDisplayName='" + this.selectedDmeDataObj.modelName + "' maxCount='"+this.chooseLayoutSavedDataObj.maxCount+ "' freeStyle='" +this.chooseLayoutSavedDataObj.freeStyle +"' parameterValues='" + parameterValues + "' rowStyle='' layoutStyle='' sortAtrributes='"+sortedAttributesObj+"' data-isDmeBlock='0'>";
    jsonObject = {
      "type": "dme",
    ...
    dynamicTemplateInsert = "<dynamic-content id='"+uuid+"' type='dme' layout='layout0' maxCount='1' modelName='" + this.selectedDmeDataObj.modelNameHash + "' modelDisplayName='" + this.selectedDmeDataObj.modelName + "' parameterValues='"+parameterValues+"' sortAtrributes='"+sortedAttributesObj+"' data-isDmeBlock='0'>";
    jsonObject = {
      "type": "dme",      
      "input_params": parameterValues,
      "modelName":this.selectedDmeDataObj.modelName,
